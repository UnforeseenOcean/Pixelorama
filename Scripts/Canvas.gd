...
func _input(event : InputEvent) -> void:
	...
	match current_action: # Handle current tool
		"Pencil":
			pencil_and_eraser(sprite, mouse_pos, current_color, current_mouse_button)
		"Eraser":
			pencil_and_eraser(sprite, mouse_pos, Color(0, 0, 0, 0), current_mouse_button)
		"Bucket":
			if can_handle:
...
func pencil_and_eraser(sprite : Image, mouse_pos : Vector2, color : Color, current_mouse_button : String, current_action := "None") -> void:
	...
	if is_making_line:
		...
		draw_pixel(sprite, line_2d.points[1], color, current_mouse_button, current_action)
		made_line = true
	else:
		if point_in_rectangle(mouse_pos, location, location + size):
			mouse_inside_canvas = true
			# Draw
			draw_pixel(sprite, mouse_pos, color, current_mouse_button, current_action)
...
func draw_pixel(sprite : Image, pos : Vector2, color : Color, current_mouse_button : String, current_action := "None") -> void:
	if Global.can_draw && Global.has_focus:
		var brush_size := 1
				...
				for cur_pos_y in range(start_pos_y, end_pos_y):
					if point_in_rectangle(Vector2(cur_pos_x, cur_pos_y), Vector2(west_limit - 1, north_limit - 1), Vector2(east_limit, south_limit)):
						...
						if current_pixel_color != color && !(pos_floored in lighten_darken_pixels):
							...
							layers[Global.current_layer][0].set_pixel(cur_pos_x, cur_pos_y, color)
							sprite_changed_this_frame = true
							...
							if horizontal_mirror:
								...
								if current_pixel_color != color: # don't draw the same pixel over and over
									...
									layers[Global.current_layer][0].set_pixel(mirror_x, cur_pos_y, color)
									sprite_changed_this_frame = true
							if vertical_mirror:
								...
								if current_pixel_color != color: # don't draw the same pixel over and over
									...
									sprite.set_pixel(cur_pos_x, mirror_y, color)
									sprite_changed_this_frame = true
							if horizontal_mirror && vertical_mirror:
								...
								if current_pixel_color != color: # don't draw the same pixel over and over
									...
									sprite.set_pixel(mirror_x, mirror_y, color)
									sprite_changed_this_frame = true
...
func fill_gaps(sprite : Image, mouse_pos : Vector2, prev_mouse_pos : Vector2, color : Color, current_mouse_button : String, current_action := "None") -> void:
	...
	while !(x == mouse_pos_floored.x && y == mouse_pos_floored.y):
		draw_pixel(sprite, Vector2(x, y), color, current_mouse_button, current_action)
		e2 = err << 1
		if e2 >= dy:
			err += dy
			x += sx
